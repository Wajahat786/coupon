<article class="detail">
  <a class="back-link" href="/referrals"><?= icon("arrow-left", 15) ?> Back to referral links</a>
  <div class="detail-head">
    ...
    <?php if (!empty($referral['reward_text'])): ?>
      <p class="chip chip-reward chip-lg"><?= icon("gift", 16) ?> <?= e($referral['reward_text']) ?></p>
    <?php endif; ?>
  </div>
  ...
  <div class="reveal-box">
    <div class="code-row code-row-lg">
      ...
      <button class="btn btn-copy" type="button" data-copy="<?= e($referral['link']) ?>"><?= icon("copy", 16) ?> Copy Link</button>
    </div>
    <a class="btn btn-primary btn-lg" href="/go/referral/<?= (int) $referral['id'] ?>" target="_blank" rel="noopener nofollow ugc">
      Open Referral Page <?= icon("external", 16) ?>
    </a>
  </div>
